      ...
      <div className="max-w-7xl mx-auto px-4 py-20 space-y-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white leading-snug">{feature1Title}</h3>
            <p className="text-zinc-600 dark:text-zinc-400 text-lg leading-relaxed">
              {feature1Desc.includes('Board Page')
                ? <>{feature1Desc.split('Board Page')[0]}<Link to="/hsc-ssc" className="text-emerald-600 hover:underline">Board Page</Link>{feature1Desc.split('Board Page')[1]}</>
                : feature1Desc}
            </p>
          </div>
          <div className="rounded-3xl overflow-hidden shadow-2xl aspect-[4/3]">
            <img src={feature1Image} alt="HSC SSC Feature" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="order-2 lg:order-1 rounded-3xl overflow-hidden shadow-2xl aspect-[4/3]">
            <img src={feature2Image} alt="Admission Feature" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
          </div>
          <div className="order-1 lg:order-2 space-y-6">
            <h3 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white leading-snug">{feature2Title}</h3>
            <p className="text-zinc-600 dark:text-zinc-400 text-lg leading-relaxed">
              {feature2Desc.includes('Admission Page')
                ? <>{feature2Desc.split('Admission Page')[0]}<Link to="/admission" className="text-emerald-600 hover:underline">Admission Page</Link>{feature2Desc.split('Admission Page')[1]}</>
                : feature2Desc}
            </p>
          </div>
        </div>
